...
namespace Titration_Analyzer
{
    public partial class Form1
    {
        public class adddata
        {
            ...
            //Error Correction
            public int parsedataindex;
            public bool parseddatarun;
            ...
            public void ParseData(double datacutoff)
            {
                parsedataindex = 0;

                foreach(var datapoint in titrationdatastorage.titrationstorage)
                {
                    var datavaluestring = datapoint.Split(',');
                    var numericalvalue = Convert.ToDouble(datavaluestring[0]);

                    if(numericalvalue < datacutoff)
                    {
                        parsedataindex ++;
                    }
                    else if(numericalvalue >= datacutoff)
                    {
                        break;
                    }
            ...
            public void findgreatest()
            {
                ...
                if(parseddatarun == true)
                {
                    derivativepeaks.Clear();
                    equivalencepointPH.Clear();
                    equivalencepoints.Clear();
                    PKavalues.Clear();
                    pKavolumes.Clear();
                    tempderivativelist.RemoveRange(0, parsedataindex);
                    tempsecondderivative.RemoveRange(0, parsedataindex);
                    tempaveragevolume.RemoveRange(0, parsedataindex);
                }
                ...
                for (int i = 0; i < passes; i++)
                {
                    ...
                    //if (parseddatarun == true)
                    //{
                    //    tempderivativelist.Clear();

                    //    tempderivativelist.AddRange(PhDerivative);
                    //}
            ...
            public bool baseoracidcheck()
            {
                
                int datasize = titrationdatastorage.titrationstorage.Count;

                var firstpointstring = titrationdatastorage.titrationstorage[0].Split(',');
                var firstpointdouble = Convert.ToDouble(firstpointstring[1]);

                var lastpointstring = titrationdatastorage.titrationstorage[datasize - 1].Split(',');
                var lastpointdouble = Convert.ToDouble(lastpointstring[1]);

                if (firstpointdouble > lastpointdouble)
                {
                    return true;
                }
                else
            ...
            public double getmaxvolume()
            {
                ...
                titrationdatastorage.maxPH = Convert.ToDouble(maxvolumestring[1]);
                titrationdatastorage.maxVolume = Convert.ToDouble(maxvolumestring[0]);
            ...
            public double GetEquivalenceManual(List<double> secondderivative, List<double> averagevolume)
            {
                ...
                titrationdatastorage.maxminfail = false;

                return equivalncepointintercept;

            Exit:
                {
                    titrationdatastorage.maxminfail = true;
                    return 0;
                }
            }
